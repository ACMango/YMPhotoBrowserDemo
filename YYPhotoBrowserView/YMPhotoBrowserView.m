...
- (id)copyWithZone:(NSZone *)zone {
    YMPhotoGroupItem *item = [self.class new];
    return item;
}
@end


#pragma mark - YMPhotoGroupCell
@interface YMPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) YMPhotoGroupItem *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation YMPhotoGroupCell
...
- (void)setItem:(YMPhotoGroupItem *)item {
...
@interface YMPhotoBrowserView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation YMPhotoBrowserView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    NSLog(@"%@", _toContainerView);
    NSInteger page = -1;
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((YMPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    YMPhotoGroupCell *cell = [self cellForPage:currentPage];
    YMPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            YMPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                YMPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (YMPhotoGroupCell *)dequeueReusableCell {
    YMPhotoGroupCell *cell = nil;
    for (cell in _cells) {
...
- (YMPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (YMPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    YMPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
